...
def transparency_check(MASTERPATH, SLAVEPATH):
    ...
    m = mse(IMAGES[0], IMAGES[1])
    s = structural_similarity(IMAGES[0], IMAGES[1])

    if s < 0.95:
        print("Structural Similarity: {ssm:.2f} %".format(ssm=s * 100))
        fig = plt.figure(MASTERPATH.replace("master", ""), [10, 8])
        plt.suptitle("Structural Similarity: {ssm:.2f} %".format(ssm=s * 100))
